...
## 在 menu 页新增分类和商品
点单页现为完全数据驱动，数据源位于 `miniprogram/data/catalog.js`：

- 新增分类：向 `categories` 追加 `{ id, name }`（id 建议英文短横线写法，例如 `cheese-cake`）。
- 新增商品：向 `products` 追加 `{ id, categoryId, name, price, brief, cover }`，其中 `categoryId` 指向对应分类的 id。
...
- 加购/减购按钮与底部购物车联动，本地持久化于 `wx.setStorageSync('cart')`。
