...
// Config
const EXCEL_PATH = process.env.BASQUE_CAKE_EXCEL || '/Users/yipengli/Desktop/cake_name3.xlsx';
...
const CATEGORY_ID = 'basque-cake';
const CATEGORY_NAME = '巴斯克蛋糕';
const ASSET_DIR = path.resolve(__dirname, '..', 'miniprogram', 'assets', 'basque-cake');
...
async function main(){
  ...
  try{ const tmp=path.resolve(__dirname,'..','tmp'); ensureDir(tmp); fs.writeFileSync(path.join(tmp,'basque-cake-source.json'), JSON.stringify(sourceMap,null,2)); }catch(_){ }
  console.log(`[BASQUE] rows:${outProducts.length}, assets:${ASSET_DIR}`);
}
