...
async function main(){
  ...
  for(let i=0;i<starters.length;i++){
    ...
    const images=[]; for(let k=0;k<hits.length;k++){ const found=hits[k].media.find(m=>m&&m.index===hits[k].img.imageId); if(!found) continue; const buf=found.buffer||(found.base64?Buffer.from(found.base64,'base64'):null); if(!buf) continue; const ext0=(found.extension||found.type||'jpeg').toLowerCase(); const ext=ext0==='jpg'?'jpeg':ext0; const fn=`cake-accessories-${slug}-${k+1}.${ext}`; fs.writeFileSync(path.join(ASSET_DIR,fn),buf); images.push(assetUrl('cake-accessories', fn));}
    outProducts.push({ id:`cake-accessories-${slug}`, categoryId:CATEGORY_ID, name:displayName, brief, images, cover: images[0]|| (ASSET_BASE_URL? assetUrl('', 'p1.jpg'):'/assets/p1.jpg'), price:Number(price), variants: variants.length?variants:[{size:'默认',price:Number(price)}] });
  }
