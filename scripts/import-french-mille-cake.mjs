...
// Config
const EXCEL_PATH = process.env.FRENCH_MILLE_EXCEL || '/Users/yipengli/Desktop/cake_name3.xlsx';
const SHEET_INDEX = 1; // 1-based
const ROW_START = 30;
const ROW_END = 39;
const COL_B = 2; // raw name
const COL_C = 3; // brief sizes (not used in parse)
const COL_D = 4; // images
const COL_E = 5; // price

const CATEGORY_ID = 'french-mille-cake';
const CATEGORY_NAME = '法式千层蛋糕';
const ASSET_DIR = path.resolve(__dirname, '..', 'miniprogram', 'assets', 'french-mille-cake');
const CATALOG_PATH = path.resolve(__dirname, '..', 'miniprogram', 'data', 'catalog.js');

// Optional fallback dir
const SRC_DIR_ENV = process.env.FRENCH_MILLE_IMG_DIR ? String(process.env.FRENCH_MILLE_IMG_DIR) : '';
const FALLBACK_DIRS = [SRC_DIR_ENV, path.resolve(__dirname, '..', 'tmp', 'french-mille-cake-src')]
  .filter(Boolean)
...
function getEmbeddedImagesForRow(sheet, workbook, rowIndex){
  const row0=rowIndex-1; const hits=[]; const imgs=sheet.getImages?sheet.getImages():[]; const media=(workbook.model&&workbook.model.media)||[];
  for(const img of imgs){ const range=img.range||{}; const tl=range.tl||range||{}; const br=range.br||range.tl||range||{}; const tlc=(tl.nativeCol??tl.col??range.col??0); const tlr=(tl.nativeRow??tl.row??range.row??0); const brc=(br.nativeCol??br.col??tlc); const brr=(br.nativeRow??br.row??tlr); const rowHit=tlr<=row0&&brr>=row0; if(!rowHit) continue; hits.push({img,tlc,tlr}); }
  hits.sort((a,b)=> (a.tlr-b.tlr) || (a.tlc-b.tlc));
  const out=[]; for(const h of hits){ const found=media.find(m=>m&&m.index===h.img.imageId); if(!found) continue; const buffer=found.buffer||(found.base64?Buffer.from(found.base64,'base64'):null); if(!buffer) continue; const ext0=(found.extension||found.type||'jpeg').toLowerCase(); const ext=ext0==='jpg'?'jpeg':ext0; out.push({buffer,ext}); }
  return out;
}
...
async function main(){
  ...
  try{ const tmp=path.resolve(__dirname,'..','tmp'); ensureDir(tmp); fs.writeFileSync(path.join(tmp,'french-mille-source.json'), JSON.stringify(sourceMap,null,2)); }catch(_){ }
  console.log(`[FRENCH] rows:${outProducts.length}, assets:${ASSET_DIR}`);
}
