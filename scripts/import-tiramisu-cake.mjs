...
async function main(){
  ...
  for(let i=0;i<starters.length;i++){
    ...
    let images=[]; if(hits.length){ hits.forEach((h,idx)=>{ const found=media.find(m=>m&&m.index===h.img.imageId); if(!found) return; const buffer=found.buffer||(found.base64?Buffer.from(found.base64,'base64'):null); if(!buffer) return; const ext0=(found.extension||found.type||'jpeg').toLowerCase(); const ext=ext0==='jpg'?'jpeg':ext0; const fn=`tiramisu-cake-${slug}-${idx+1}.${ext}`; fs.writeFileSync(path.join(ASSET_DIR,fn), buffer); images.push(assetUrl('tiramisu-cake', fn)); }); sourceMap[`tiramisu-cake-${slug}`]='excel'; } else { sourceMap[`tiramisu-cake-${slug}`]='none'; }
