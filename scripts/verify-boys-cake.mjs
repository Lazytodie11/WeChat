...
function main(){
  ...
  const list = products.filter(p=>p.categoryId==='boys-cake');
  let anomalies=0, excel=0, fallback=0, none=0, totalImgs=0; const seen=new Map();
  let srcMap={}; try{ srcMap=JSON.parse(fs.readFileSync(path.resolve(__dirname,'..','tmp','boys-cake-source.json'),'utf8')); }catch(_){ }
  for(const p of list){
    const imgs=Array.isArray(p.images)?p.images:[]; totalImgs+=imgs.length;
    const allInDir=imgs.length>0 && imgs.every(s=>typeof s==='string' && s.startsWith('/assets/boys-cake/'));
