...
function main(){
  ...
  const list = products.filter(p=>p.categoryId==='cake-accessories');
  let anomalies=0, totalImgs=0; const seen=new Map();
  for(const p of list){
    const imgs=Array.isArray(p.images)?p.images:[]; totalImgs+=imgs.length;
    const allInDir=imgs.length>0 && imgs.every(s=>typeof s==='string' && s.startsWith('/assets/cake-accessories/'));
    ...
    console.log(`[OK] ${p.name} | imgs:${imgs.length} inDir:${allInDir} coverOk:${coverOk} priceMin:${priceMin} price:${p.price} nameClean:${nameOk} briefOk:${briefOk}`);
  }
  console.log(`Total products: ${list.length}, images total: ${totalImgs}, anomalies: ${anomalies}`);
}
